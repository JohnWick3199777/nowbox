...
def _example_files(root: Path) -> list[str]:
    return sorted(p.name for p in (root / "examples").glob("*.py"))


@command("run")
@mode("dev")
@argument("file", default="example.py", choices=lambda: _example_files(Path(".")), help="Example file to run")
...
def run(ctx: Context) -> None:
    ...
    script = ctx.project_root / "examples" / ctx.args["file"]
